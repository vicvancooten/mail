...
- **Amended by [ADR-0023](./0023-app-data-rides-the-sync-protocol-under-sync-scopes.md)**: every collection declares a Sync Scope (User, Mail Account or Connected Account), the collection list grows per Hub App, `Label` moves from Mail Account scope to User scope, and a `connectedAccounts` sibling joins `user` and `mailAccounts` additively rather than restructuring the request. Note bodies travel on a `noteSaves` channel shaped like `composeSaves`, not as mutations.
