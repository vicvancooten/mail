 ...
 * given per-App Account Scope in #187): Mail, live today, plus four reserved
 * Apps that are named and reachable — never hidden — long before anything is
 * built behind them. `available: false` is what routes a click to
 * `PlaceholderRoute` instead of a real screen; it is not a disabled state,
 * since the whole point of naming a reserved App is that it stays a real,
 * clickable destination. Notes is last (`docs/hub-apps-foundations-spec.md`'s
 * own App order) — reserved the same way Contacts/Calendar/Tasks were before
 * it, until Notes' own epic (`mail#190`) lands behind it.
 */
