...
const envSchema = z.object({
  ...
  /**
   * #213's own instance-level Contact photo budget, in raw bytes — 5MB
   * default (`@mail/shared#CONTACT_PHOTO_MAX_BYTES`). Read once at boot and
   * handed to `routes/contact-photos.ts`; never a per-User setting, the same
   * posture `ATTACHMENT_BUDGET_BYTES` already takes.
   */
  CONTACT_PHOTO_BUDGET_BYTES: z.coerce
    .number()
    .int()
    .positive()
    .default(5 * 1024 * 1024),
