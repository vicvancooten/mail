...
/** Matches Gmail's own wording for the daily bandwidth/download cap, seen in the wild as both phrasings. */
const DOWNLOAD_CAP_PATTERN = /bandwidth limit|command or bandwidth limits/i;
 ...
 * an ordinary connection or protocol failure. Only meaningful for a
 * `serverKind === "gmail"` account (`body-sweep.ts`'s only caller already
 * gates on that) — the same wording is neither expected nor specially
 * handled from any other server (ADR-0020).
 */
