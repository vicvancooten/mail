import type { NoteSave, NoteSaveOutcome } from "@mail/shared";
...
/**
 * Applies one User's queued Note body autosaves (#192, ADR-0023) — the
 * `noteSaves` channel, `compose-store.ts#flushComposeSaves`'s sibling and
 * deliberately simpler: `saves` carries at most one entry per Note (the
 * Client's own coalescing queue, `store/notes.ts`, never holds more than
 * that), and — unlike a Composition save — there is no version to check and
 * no ledger to replay against.
 *
 ...
 * the whole of what "never rejects a Note write" (ADR-0023) buys here — no
 * `note_save_ledger` table, no race to reconcile.
 */
export async function flushNoteSaves(
  ...
  const outcomes: NoteSaveOutcome[] = [];
 ...
 * `createNote`: the `noteSaves` channel and the User-scoped mutation queue
 ...
 */
async function applyOne(db: Db, userId: string, save: NoteSave): Promise<NoteSaveOutcome> {
  ...
  return { id: save.id, saveId: save.saveId, status: "applied" };
}
