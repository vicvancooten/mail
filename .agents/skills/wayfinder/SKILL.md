...
5. Commit the session's work to the current branch: **one ticket, one commit**, written as a [Conventional Commit](https://www.conventionalcommits.org/) (`feat:`, `fix:`, `docs:`, `chore:` …) whose subject closes the ticket it resolves. No agent self-attribution in the message — no `Co-Authored-By` trailer, no "generated with" footer. Don't push; the human handles that.
